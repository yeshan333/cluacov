...
```
   12         :   1 : end           ← 函数的 end： 可执行，命中 1 次
   25         :     :    end        ← if 块的 end：不可执行（留空）
   42         :   0 :    end        ← for 循环的 end：可执行，命中 0 次
```

这**不是 cluacov 的 bug**——而是 Lua 字节码层面的客观事实。一行源码会被
标记为"可执行"，当且仅当 Lua 编译器在编译时**真的为这一行发射了至少一条
字节码指令**（即该指令的 `lineinfo` 记录指回这一行）。`end` 关键字本身
并不是任何意义上的"语句"；它能不能进入行表，**完全取决于编译器选择把
什么样的控制流指令绑定到这一行上**：

| `end` 的位置 | 绑定到该行的字节码 | 是否可执行 |
|-------------|-------------------|:---------:|
| **函数体的 `end`** | `OP_RETURN`（隐式 `return nil`） | ✅ 是 |
| **`for` 循环的 `end`** | `OP_FORLOOP`（回跳到循环头） | ✅ 是 |
| **`while` 循环的 `end`** | `OP_JMP`（回跳到 `while` 条件判断） | ✅ 是 |
| **`repeat` 的 `until`** | until 条件的 `OP_TEST` | ✅ 是（绑定到 `until` 行） |
| **`if`/`elseif`/`else` 块的 `end`** | *（无——编译器不在此处发射任何指令）* | ❌ 否 |
| **`do ... end` 块的 `end`** | *（无）* | ❌ 否 |

直觉理解：循环和函数的 `end` 对应着真实的运行时动作（一次 return，或一次
回跳）；而 `if-end` 和 `do-end` 纯粹是语法标记——周围 `then`/`else` 分支
上的 `OP_JMP` 指令已经处理好了控制流，根本不需要"执行" `end` 这一行。

你可以用 `luac -l -p file.lua` 反汇编任何函数来亲自验证：

```
[L42]  23  TFORLOOP  ...    ← 绑定在第 42 行（for 循环的 end）
[L44]  24  RETURN    ...    ← 绑定在第 44 行（函数的 end）
                            ←（没有指令绑定在第 41 行——
                               即 if 块的 end——所以那一行留空）
```

简单总结：**报告里所有有命中次数的 `end`，背后都对应着一条真实的控制流
指令；所有留空的 `end`，仅仅是因为编译器没有理由在那里发射任何代码。**
这与 `luacov`、`lcov`/`gcov` 以及其他 Lua 覆盖率工具的渲染方式保持一致，
也让"覆盖行数"的分母更诚实——如果把空白的 `end` 都算成"未覆盖行"，
反而会人为压低每个项目的覆盖率指标。
