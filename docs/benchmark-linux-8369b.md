...
|---|---|---|---|---|
| fib(24) | 2.846e+02 | 2.115e+02 | 9.461e+02 | 0.74x |
| loop×1 000 | 2.661e+07 | 3.626e+07 | 1.863e+08 | 1.36x |
| call-chain/100 | 4.128e+05 | 3.134e+05 | 1.204e+06 | 0.76x |
...
|---|---|---|---|---|
| fib(24) | luacov-hook | 175.5x | 181.4x | 188.8x |
| fib(24) | cluacov-hook | 22.3x | 24.7x | 28.7x |
| fib(24) | pchook | 24.1x | 44.8x | 50.0x |
| loop×1 000 | luacov-hook | 287.0x | 539.7x | 649.1x |
| loop×1 000 | cluacov-hook | 33.1x | 65.7x | 90.8x |
| loop×1 000 | pchook | 17.0x | 56.7x | 69.1x |
| call-chain/100 | luacov-hook | 227.2x | 240.8x | 218.8x |
| call-chain/100 | cluacov-hook | 27.4x | 31.0x | 31.2x |
| call-chain/100 | pchook | 25.7x | 48.0x | 45.6x |

### Geometric mean slowdown

| Mode | Xeon 8269CY | Xeon 8369B | M4 Pro |
|---|---|---|---|
| luacov-hook | 225.3x | 286.7x | 299.3x |
| cluacov-hook | 27.2x | 36.9x | 43.3x |
| pchook | 21.9x | 49.6x | 54.0x |

### Head-to-head winner (cluacov-hook vs pchook)

| | Xeon 8269CY | Xeon 8369B | M4 Pro |
|---|---|---|---|
| fib(24) | cluacov-hook (1.08x) | cluacov-hook (1.81x) | cluacov-hook (1.74x) |
| loop×1 000 | pchook (1.95x) | pchook (1.16x) | pchook (1.31x) |
| call-chain/100 | pchook (1.07x) | cluacov-hook (1.55x) | cluacov-hook (1.46x) |
| **geometric mean** | **pchook (1.24x)** | **cluacov-hook (1.34x)** | **cluacov-hook (1.25x)** |

## Analysis

### Key observations

1. **pchook loses its advantage on this machine**: On the original Xeon 8269CY,
   pchook's geometric mean slowdown (21.9x) beat cluacov-hook (27.2x) by 1.24x.
   On the Xeon 8369B, the relationship **reverses** — cluacov-hook (36.9x) beats
   pchook (49.6x) by 1.34x. This mirrors the macOS result where cluacov-hook
   also wins (1.25x).

2. **Slowdown ratios are higher than the original Linux result**: The Xeon 8369B
   shows significantly higher hook overhead than the 8269CY (e.g., pchook 49.6x
   vs 21.9x), despite comparable baseline throughput. This is consistent with the
   observation from the macOS benchmark: when the uninstrumented code runs faster
   (or the per-event hook cost is higher relative to baseline), the fixed hook
   cost becomes a larger relative penalty.

3. **pchook disproportionately affected**: pchook's slowdown increases from 21.9x
   to 49.6x (2.27x increase) while cluacov-hook only increases from 27.2x to
   36.9x (1.36x increase). pchook fires 3–7x more events than the line hook; the
   cumulative effect of these extra events is amplified on machines where the
   baseline is efficient enough that the per-event overhead dominates.

4. **Only loop×1 000 favors pchook**: Across all three non-8269CY machines,
...
- The original Linux benchmark (Xeon 8269CY) appears to be an outlier in favoring
  pchook. On two other machines (Xeon 8369B, M4 Pro), cluacov-hook is faster
  overall.
- The performance gap between cluacov-hook and pchook is moderate in either
  direction (1.16–1.95x per workload), so the choice should be guided primarily
  by **feature requirements** (pchook enables PC-level branch coverage) rather
  than performance alone.
- Both C hooks are dramatically faster than the pure-Lua luacov hook on all
  tested machines (4–10x improvement).
