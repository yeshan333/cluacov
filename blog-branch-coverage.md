...
[cluacov](https://github.com/yeshan333/cluacov) 是 [LuaCov](https://github.com/lunarmodules/luacov)（Lua 生态中最主流的代码覆盖率工具，纯 Lua 实现）的 C 扩展。最初版本的 cluacov 仅用于提供行级覆盖率的性能加速，本身并不支持分支覆盖。为了填补这一空白，我们设计并实现了两套分支覆盖率方案：一套基于行级调试钩子的近似方案（兼容 Lua 5.1 到 5.5 及 LuaJIT），另一套基于指令级计数钩子的精确方案（仅支持 Lua 5.4+）。两者的设计差异，本质上反映了 Lua 调试接口在不同粒度下的能力边界。

![cluacov 架构对比：行级方案 vs 指令级方案](blog-images/01-architecture-comparison.png)
...
- **注册表 (Registry)**：一个只有 C 代码能访问的全局 Lua 表，用于存储不想暴露给 Lua 层的私有数据。cluacov 用注册表存储每个 Proto 的命中计数表
...
在不支持指令级钩子的环境下，使用传统的 LuaCov 行命中数据配合 [deepbranches](https://github.com/yeshan333/cluacov/blob/master/src/cluacov/deepbranches.c) 静态分析：

```sh
lua -lluacov your_program.lua
```

然后用 [deepbranches.get](https://github.com/yeshan333/cluacov/blob/master/src/cluacov/deepbranches.c) 发现分支点，与行命中数据交叉比对来估算分支覆盖率。由于行级钩子的粒度限制，需要经过 [branchfilter.lua](https://github.com/yeshan333/cluacov/blob/master/src/cluacov/branchfilter.lua) 过滤无法区分的同行分支。
...
1. **Lua 接口的能力限制**：Lua 标准调试库（`debug`）仅向用户层暴露了 `line`（行）、`call`（调用）、`return`（返回）和 `count`（指令数计数）事件，并未向 Lua 提供类似“当前指令 PC 值”或“执行的操作码”等更细粒度的机制。
2. **调试钩子的物理边界**：在行触发模式（`LUA_MASKLINE`）下，回调函数仅能获得当前正在执行的“行号”，根本无法感知或区分这一物理行内到底执行了哪条具体的字节码指令。
3. **巨大的运行时性能开销**：原生的 LuaCov 回调函数完全基于纯 Lua 编写。这意味着每当程序运行一行，就要触发一次 Lua 层面的函数调用，由此引入了 200x~300x 级别的运行开销。在 Lua 用户层已经极为缓慢的前提下，根本无法支持更细粒度的高频指令监听。

我们实现的**旧架构（行级近似方案）**，正是保留了这种“行级数据”的统计模式，但在 C 语言层面重写了 Hook 以提升性能（即 [hook.c](https://github.com/yeshan333/cluacov/blob/master/src/cluacov/hook.c) ），然后利用静态分析的分支行号进行估算对齐。其计算逻辑如下：

1. `deepbranches.get(func)` 静态发现所有分支点及其两个目标的行号
...
![if a or b or c then 的字节码流与分支可见性对比](blog-images/02-or-bytecode-flow.png)
...
对于同一行上有多个分支点的情况，只保留**两个目标行都不在本行**的分支。同时，对目标行对相同的分支做去重。
...
解决方案是**惰性拷贝快照**（snapshot on first execute，即首次遇到该 Proto 时立即将其元数据复制备份为纯 Lua 表）：在钩子第一次遇到某个 Proto 时，立即将所有后续需要的元数据复制到纯 Lua 表中，此后再也不需要访问 Proto 指针。

![pchook.c 数据生命周期：Proto 惰性拷贝与 GC 安全](blog-images/03-pchook-lifecycle.png)
...
![deepbranches.c 字节码分支发现流程](blog-images/04-deepbranches-scan.png)
...
| 维度 | 行级方案（[hook.c](https://github.com/yeshan333/cluacov/blob/master/src/cluacov/hook.c)） | 指令级方案（[pchook.c](https://github.com/yeshan333/cluacov/blob/master/src/cluacov/pchook.c)） |
|------|-------------------|----------------------|
...
| 代码量 | ~550 行 C（[hook.c](https://github.com/yeshan333/cluacov/blob/master/src/cluacov/hook.c) 179 行 + [deepbranches.c](https://github.com/yeshan333/cluacov/blob/master/src/cluacov/deepbranches.c) 375 行） | ~1190 行 C + Lua（[pchook.c](https://github.com/yeshan333/cluacov/blob/master/src/cluacov/pchook.c) 1114 行 + [branchcov.lua](https://github.com/yeshan333/cluacov/blob/master/src/cluacov/branchcov.lua) 76 行；[deepbranches.c](https://github.com/yeshan333/cluacov/blob/master/src/cluacov/deepbranches.c) 共用） |
...
## 十三、 在 Aone CI 插件中的工程落地

为了将静态分析和指令级动态钩子完美应用于自动化工作流，目前该能力已深度集成于我们的 Aone CI 流水线插件中。

### 13.1 插件定位与实现
该能力集成在 Aone CI 流水线的 Unite 组件中，组件地址为：
👉 `[Aone CI Unite 组件: lua-busted](https://cd.aone.alibaba-inc.com/unite/component/1364/detail)`

插件在 CI 流水线中封装了基于 `busted` 测试框架的运行套件，在单测启动阶段自动化加载 `cluacov` 原生 native 模块。其主要运行机制如下：
1. **自动前置挂钩**：在测试执行前注入配置，通过 `pchook.start()` 启动指令级高频监控（Lua 5.4+）或 Line Hook（其他版本）。
2. **测试驱动与执行**：自动调用类似于 `rtk busted -o htest` 的流水线标准单测指令运行用例。
3. **后置对齐与生成**：在测试结束时调用 `pchook.stop()`。随后自动触发 `runner.lua` 对代码库中的所有被测源文件调用 `loadfile()` 编译成 Proto，启动静态扫描器 `deepbranches.c` 对其进行分支位置定位，最终将运行时记录的 PC 命中图与分支目标 PC 进行交叉比对，将结果合并转换并持久化为标准的 `lcov.info` 格式，特别包含带有 `BRDA` 分支统计数据的段落。

### 13.2 CI 门禁与覆盖率渲染
Aone 平台在解析到包含 `BRDA` 记录的 `lcov.info` 文件后，会自动将其解析并呈现在 Merge Request 页面或质量大盘中。
* 开发人员无需修改单测逻辑即可获取分支级洞察。
* 流水线的质量门禁（Quality Gates）不仅能对行覆盖率进行卡点，更能限制分支覆盖率的下限值，彻底阻断由于逻辑短路（如 `or` / `and` 复合判断）导致的覆盖不完整代码上线。
...
![附录：如何解读 LCOV 覆盖率报告](blog-images/05-lcov-report-guide.png)
...
![genhtml 生成的分支覆盖率 HTML 报告](blog-images/06-genhtml-report.png)
